...
class HexapodKinematics(object):
    ...
    def getLegIK(self,xd):
        ...
        angles[6:9] = self.rfLeg.solve_inverse_kinematics(guess[3:6], ee_pos_objective2)

        ee_pos_objective3 = hebi.robot_model.endeffector_position_objective(xd[:,2])
        angles[12:15] = self.lmLeg.solve_inverse_kinematics(guess[6:9], ee_pos_objective3)

        ee_pos_objective4 = hebi.robot_model.endeffector_position_objective(xd[:,3])
        angles[3:6] =self.rmLeg.solve_inverse_kinematics(guess[9:12], ee_pos_objective4)

        ee_pos_objective5 = hebi.robot_model.endeffector_position_objective(xd[:,4])
        angles[9:12] = self.lbLeg.solve_inverse_kinematics(guess[12:15], ee_pos_objective5)
